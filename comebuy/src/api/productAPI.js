...
const productAPI = {
    ...
    edit: async (data) => {
        const res = await DatabaseClient.put(`/${baseURL}/${data.productID}`, data).catch((err) => err.response);
        return res;
    },
    ...
    createNewProduct: async (newProduct) => {
        const res = await DatabaseClient.post(`/${baseURL}`, newProduct).catch((err) => err.response);
        return res;
    },
    deleteByID: async (productID) => {
        const res = await DatabaseClient.delete(`/${baseURL}/${productID}`).catch((err) => err.response);
        return res;
    },
    getProductFilterOptions: async () => {
        const res = await DatabaseClient.get(`/${baseURL}/filter/feature`).catch((err) => err.response);
        return res;
    },
