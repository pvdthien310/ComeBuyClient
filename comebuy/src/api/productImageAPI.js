...
const productImageAPI = {
    ...
    addMany: async (list) => {
        const res = await DatabaseClient.post(`/${baseURL}/many`, list).catch((err) => err.response);
        return res;
    },
    deleteImagesOfProduct: async (productID) => {
        const res = await DatabaseClient.delete(`/${baseURL}/byproductid/${productID}`).catch((err) => err.response);
        return res;
    },
};
