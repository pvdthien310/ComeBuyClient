...
const invoiceAPI = {
    ...
    updateInvoice: async (data) => {
        const res = await DatabaseClient.put(`/${baseURL}/${data.invoiceID}`, data).catch((err) => err.response);
        return res.data;
    },
    RevenueByBranch: async (branchID) => {
        const res = await DatabaseClient.get(`/${baseURL}/revenueByBranchID/${branchID}`).catch((err) => err.response);
        return res;
    },
