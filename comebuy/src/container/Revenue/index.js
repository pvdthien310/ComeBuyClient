    ...
    return (
        <Box sx={{ width: '100%', height: '100%' }}>
            <Box sx={{ bgcolor: '#2e1534' }}>
                <StyledTabs value={value} onChange={handleChange} variant="scrollable" aria-label="styled tabs example">
                    {branchList.length > 0 &&
                        branchList.map((item, index) => <StyledTab key={index} label={item.address} />)}
                </StyledTabs>
                <Box sx={{ p: 1 }} />
            </Box>
            {branchList.length > 0 &&
